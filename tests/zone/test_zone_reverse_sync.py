...
def test_forward_a_record_creates_managed_reverse_zone(tmp_path: Path) -> None:
    ...
    m.add_record("example.com", "A:www:192.168.100.10", ttl=300)
...
def test_forward_a_record_update_updates_reverse_zone(tmp_path: Path) -> None:
    ...
    m.add_record("example.com", "A:app:192.168.100.10")
    m.update_record("example.com", "A:app:192.168.100.10:192.168.101.20")
...
def test_forward_a_record_delete_removes_empty_managed_reverse_zone(tmp_path: Path) -> None:
    m = manager(tmp_path)
    m.create("example.com", "master", ["external", "internal"])
    m.add_record("example.com", "A:www:192.168.100.10")
    m.delete_record("example.com", "A:www:192.168.100.10")
