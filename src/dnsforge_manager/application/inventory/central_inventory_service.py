...
class CentralInventoryService:
    ...
    def aggregate_readiness(self) -> dict[str, object]:
        ...
        aggregate = max((status.readiness for status in statuses), key=lambda value: order[value], default=AgentReadiness.READY)
        return {
            "status": aggregate.value,
            "agents": [status.to_dict() for status in statuses],
            "summary": {state.value: sum(1 for item in statuses if item.readiness == state) for state in AgentReadiness},
        }
