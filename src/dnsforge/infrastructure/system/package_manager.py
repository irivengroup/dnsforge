...
class PackageManager:
    ...
    def install_bind(self,dry_run:bool=False)->list[list[str]]:
        ...
        if not dry_run:
            for cmd in commands: self.runner.run(cmd,check=True,timeout=600)
        return commands
    def _commands_for(self,distro:LinuxDistribution)->list[list[str]]:
        if distro.family=="redhat": return [["dnf","install","-y","bind","bind-utils"]]
        if distro.family=="debian": return [["apt-get","update"],["apt-get","install","-y","bind9","bind9-utils","dnsutils"]]
        if distro.family=="suse": return [["zypper","--non-interactive","install","bind","bind-utils"]]
