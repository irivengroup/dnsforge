...
class LinuxDistribution:
    ...
    def family(self) -> str:
        ...
        if any(x in values for x in ("rhel","fedora","rocky","almalinux","centos")): return "redhat"
        if any(x in values for x in ("debian","ubuntu")): return "debian"
        if any(x in values for x in ("suse","sles","opensuse")): return "suse"
        return "unknown"
