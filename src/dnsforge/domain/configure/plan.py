...
class ConfigurePlan:
    ...
    backend: str = "legacy-bash"
    steps: list[ConfigureStep] = field(default_factory=list)

    def add_step(self, name: str, command: list[str] | None = None, description: str = "") -> None:
        self.steps.append(ConfigureStep(name=name, command=command, description=description))
    ...
    def summary(self) -> str:
        lines = [
            ...
            f"skip_install={'yes' if self.skip_install else 'no'}",
        ]
