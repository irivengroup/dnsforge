...
class AclService:
    ...
    def create(self, name: str) -> str:
        data = self._load(); data.setdefault(name, []); self._save(data)
    ...
    def add_network(self, name: str, network: str) -> str:
        ...
        data = self._load(); data.setdefault(name, [])
        if network not in data[name]: data[name].append(network)
        self._save(data)
        return f'Network added to ACL {name}: {network}'

    def remove_network(self, name: str, network: str) -> str:
        data = self._load()
        if name in data and network in data[name]: data[name].remove(network)
        self._save(data)
