...
class ViewService:
    ...
    def create(self, name: str) -> str:
        data = self._load(); data.setdefault(name, {'zones': []}); self._save(data)
    ...
    def attach_zone(self, name: str, zone: str) -> str:
        data = self._load(); data.setdefault(name, {'zones': []})
        if zone not in data[name]['zones']: data[name]['zones'].append(zone)
        self._save(data)
