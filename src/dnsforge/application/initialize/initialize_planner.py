...
class InitializePlanner:
    ...
    def build_proxy_plan(
        ...
        if backup_before_apply:
            plan.add_step("backup existing BIND configuration", description="mv live BIND config/data to /var/backups/dnsforge/bind-config/<timestamp>.tar.gz")
        plan.add_step("apply native Enterprise BIND tree", description=f"deploy {self.layout.named_conf}, {self.layout.config_dir}, {self.layout.data_dir}, {self.layout.log_dir}")
        plan.add_step("apply ownership and permissions", description="root:named for config, named:named for zones/log/runtime; strict modes")
        plan.add_step("configure firewall", ["firewall-cmd", "--permanent", "--add-service=dns"])
        plan.add_step("reload firewall", ["firewall-cmd", "--reload"])
        plan.add_step("apply selinux contexts", ["restorecon", "-Rv", *[str(path) for path in self.layout.selinux_paths]])
