...
class ClusterApi:
    ...
    def audit_cluster(self, output_format: str = "text") -> tuple[bool, str]:
        ...
        self._publish(
            "ClusterAuditCompleted", "Cluster audit OK" if ok else "Cluster audit failed", "info" if ok else "error"
        )
        return ok, output
    ...
    def _publish(self, event_type: str, message: str, severity: str = "info") -> None:
        event = AuditEvent(
            event_type=event_type, category="cluster", subject="cluster", message=message, severity=severity
        )
